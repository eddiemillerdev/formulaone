#!/usr/bin/env node
/**
 * Upload only 2027 race flag SVGs under `public/2027/race-flags/`.
 * Same env and CLI flags as upload-public-to-s3.js, with a separate manifest.
 *
 ...
 *   bun run upload-race-flags-s3 -- --dry-run
 *   bun run upload-race-flags-s3
 *   AWS_S3_UPLOAD_CONCURRENCY=48 bun run upload-race-flags-s3
 */
const fs = require("fs");
...
const DEFAULT_MANIFEST_PATH = path.join(__dirname, "public-s3-manifest-race-flags.json");
const DEFAULT_UPLOAD_LOG = path.join(__dirname, "public-s3-upload-race-flags.log");
...
/** Relative path uses forward slashes. Only `public/2027/race-flags/` (SVG assets). */
function isRaceFlagFile(rel) {
  const n = rel.replace(/\\/g, "/");
  if (!n.startsWith("2027/race-flags/")) return false;
  return /\.svg$/i.test(n);
}
...
async function main() {
  ...
  const files = allFiles.filter((abs) => {
    ...
    return isRaceFlagFile(rel);
  });
  const publicLabel = path.relative(process.cwd(), PUBLIC_DIR) || "public";
  console.log(
    `Found ${files.length} race flag file(s) under 2027/race-flags/ of ${allFiles.length} total under ${publicLabel}`,
  );
