#!/usr/bin/env node
/**
 * Upload only race carousel assets: `1.jpg`, `2.jpg`, `3.jpg` under
 * `public/backgrounds/{Grand Prix}/` and the same relative paths under
 * `public/backgrounds-thumbs/`. Same env and flags as upload-public-to-s3.js,
 * but uses a separate manifest so full-site uploads stay independent.
 *
 * Thumbnails mirror full-size filenames (optimize-background-images writes
 * backgrounds-thumbs/... with the same basename). If mains were renamed to 1–3,
 * regenerate or copy thumbs so those folders also contain 1.jpg–3.jpg; old
 * long-named thumb files are unused by the app.
 *
...
function isCarouselSlotFile(rel) {
  ...
  if (!n.startsWith("backgrounds/") && !n.startsWith("backgrounds-thumbs/")) return false;
...
async function main() {
  ...
  console.log(
    `Found ${files.length} carousel file(s) (1/2/3 under backgrounds + backgrounds-thumbs) of ${allFiles.length} total under ${publicLabel}`,
  );
