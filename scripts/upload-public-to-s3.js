#!/usr/bin/env node
/**
 * Upload everything under public/ to S3 so keys match URL paths used by the app
 * (e.g. backgrounds/..., images/..., f1.mp4). Set NEXT_PUBLIC_ASSET_BASE_URL to your
 * bucket website URL, CloudFront domain, or R2 public URL (no trailing slash).
 *
 * Incremental uploads: a JSON manifest stores sha256 per relative path. Files whose
 * content hash matches the last successful upload are skipped. Delete the manifest
 * (or use --force) to upload everything again.
 *
 ...
 *   bun run upload-public-s3 -- --dry-run
 *   bun run upload          (alias)
 *   AWS_S3_UPLOAD_CONCURRENCY=48 bun run upload
 */
const fs = require("fs");
...
const DEFAULT_MANIFEST_PATH = path.join(__dirname, "public-s3-manifest.json");
const DEFAULT_UPLOAD_LOG = path.join(__dirname, "public-s3-upload.log");
...
async function main() {
  ...
  console.log(`Found ${files.length} file(s) under ${publicLabel}`);
